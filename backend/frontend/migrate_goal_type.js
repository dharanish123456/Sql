...
const fileBase = "goal-type";
const componentName = "GoalTypePage";
...
bodyHtml = bodyHtml.replace(/href="([^"]+)\.php"/g, (match, p1) => {
  if (p1 === "dashboard") return 'to="/dashboard"'; // Special case for Link component
  return `to="/${p1}"`;
});

// Specifically fix breadcrumb links to use Link component pattern if needed,
// but for now we'll just do a global replace of <a> with Link where appropriate or just use the extracted html.
// Actually, it's better to manually wrap it in the final JSX.
