export const adminPhpRoutes = [
  {
    path: "activity",
    component: "ActivityPage",
  },
  {
    path: "admin/video-call",
    component: "VideoCallPage",
  },
  {
    path: "admin/video-call",
    component: "VideoCallPage",
  },
  {
    path: "admin-dashboard",
    component: "AdminDashboardPage",
  },
  {
    path: "admin-dashboard",
    component: "AdminDashboardPage",
  },
  {
    path: "ai-settings",
    component: "AiSettingsPage",
  },
  {
    path: "analytics",
    component: "AnalyticsPage",
  },
  {
    path: "api-keys",
    component: "ApiKeysPage",
  },
  {
    path: "approval-settings",
    component: "ApprovalSettingsPage",
  },
  {
    path: "asset-categories",
    component: "AssetCategoriesPage",
  },
  {
    path: "assets",
    component: "AssetsPage",
  },
  {
    path: "assets/dashboard",
    component: "AssetsDashboardPage",
  },
  {
    path: "attendance-admin",
    component: "AttendanceAdminPage",
  },
  {
    path: "attendance-employee",
    component: "AttendanceEmployeePage",
  },
  {
    path: "attendance-report",
    component: "AttendanceReportPage",
  },
  {
    path: "authentication-settings",
    component: "AuthenticationSettingsPage",
  },
  {
    path: "backup",
    component: "BackupPage",
  },
  {
    path: "ban-ip-address",
    component: "BanIpAddressPage",
  },
  {
    path: "blog-categories",
    component: "BlogCategoriesPage",
  },
  {
    path: "blog-comments",
    component: "BlogCommentsPage",
  },
  {
    path: "blogs",
    component: "BlogsPage",
  },
  {
    path: "blog-tags",
    component: "BlogTagsPage",
  },
  {
    path: "budget-expenses",
    component: "BudgetExpensesPage",
  },
  {
    path: "budget-revenues",
    component: "BudgetRevenuesPage",
  },
  {
    path: "budgets",
    component: "BudgetsPage",
  },
  {
    path: "bussiness-settings",
    component: "BussinessSettingsPage",
  },
  {
    path: "calendar",
    component: "CalendarPage",
  },
  {
    path: "call",
    component: "CallPage",
  },
  {
    path: "call-history",
    component: "CallHistoryPage",
  },
  {
    path: "categories",
    component: "CategoriesPage",
  },
  {
    path: "chart-apex",
    component: "ChartApexPage",
  },
  {
    path: "chart-c3",
    component: "ChartC3Page",
  },
  {
    path: "chart-flot",
    component: "ChartFlotPage",
  },
  {
    path: "chart-js",
    component: "ChartJsPage",
  },
  {
    path: "chart-morris",
    component: "ChartMorrisPage",
  },
  {
    path: "chart-peity",
    component: "ChartPeityPage",
  },
  {
    path: "chat",
    component: "ChatPage",
  },
  {
    path: "cities",
    component: "CitiesPage",
  },
  {
    path: "clear-cache",
    component: "ClearCachePage",
  },
  {
    path: "clients-grid",
    component: "ClientsPage",
  },
  {
    path: "clients",
    component: "ClientsPage",
  },
  {
    path: "coming-soon",
    component: "ComingSoonPage",
  },
  {
    path: "companies",
    component: "CompaniesPage",
  },
  {
    path: "candidates",
    component: "CandidatesPage",
  },
  {
    path: "candidates-grid",
    component: "CandidatesPage",
  },
  {
    path: "companies-grid",
    component: "CompaniesPage",
  },
  {
    path: "connected-apps",
    component: "ConnectedAppsPage",
  },
  {
    path: "contacts-grid",
    component: "ContactsGridPage",
  },
  {
    path: "contacts",
    component: "ContactsPage",
  },
  {
    path: "countries",
    component: "CountriesPage",
  },
  {
    path: "cronjob",
    component: "CronjobPage",
  },
  {
    path: "currencies",
    component: "CurrenciesPage",
  },
  {
    path: "custom-css",
    component: "CustomCssPage",
  },
  {
    path: "custom-fields",
    component: "CustomFieldsPage",
  },
  {
    path: "custom-js",
    component: "CustomJsPage",
  },
  {
    path: "daily-report",
    component: "DailyReportPage",
  },
  {
    path: "dashboard",
    component: "DashboardPage",
  },
  {
    path: "data-tables",
    component: "DataTablesPage",
  },
  {
    path: "deals-dashboard",
    component: "DealsDashboardPage",
  },
  {
    path: "deals-grid",
    component: "DealsPage",
  },
  {
    path: "deals",
    component: "DealsPage",
  },
  {
    path: "departments",
    component: "DepartmentsPage",
  },
  {
    path: "designations",
    component: "DesignationsPage",
  },
  {
    path: "domain",
    component: "DomainPage",
  },
  {
    path: "email",
    component: "EmailPage",
  },
  {
    path: "email-settings",
    component: "EmailSettingsPage",
  },
  {
    path: "email-template",
    component: "EmailTemplatePage",
  },
  {
    path: "email-verification",
    component: "EmailVerificationPage",
  },
  {
    path: "email-verification-2",
    component: "EmailVerification2Page",
  },
  {
    path: "email-verification-3",
    component: "EmailVerification3Page",
  },
  {
    path: "employee-dashboard",
    component: "EmployeeDashboardPage",
  },
  {
    path: "employee-report",
    component: "EmployeeReportPage",
  },
  {
    path: "employees",
    component: "EmployeesPage",
  },
  {
    path: "employee-salary",
    component: "EmployeeSalaryPage",
  },
  {
    path: "employees-grid",
    component: "EmployeesPage",
  },
  {
    path: "error-404",
    component: "Error404Page",
  },
  {
    path: "error-500",
    component: "Error500Page",
  },
  {
    path: "estimates",
    component: "EstimatesPage",
  },
  {
    path: "expenses",
    component: "ExpensesPage",
  },
  {
    path: "expenses-report",
    component: "ExpensesReportPage",
  },
  {
    path: "faq",
    component: "FaqPage",
  },
  {
    path: "file-manager",
    component: "FileManagerPage",
  },
  {
    path: "forgot-password",
    component: "ForgotPasswordPage",
  },
  {
    path: "forgot-password-2",
    component: "ForgotPassword2Page",
  },
  {
    path: "forgot-password-3",
    component: "ForgotPassword3Page",
  },
  {
    path: "form-basic-inputs",
    component: "FormBasicInputsPage",
  },
  {
    path: "form-checkbox-radios",
    component: "FormCheckboxRadiosPage",
  },
  {
    path: "form-fileupload",
    component: "FormFileuploadPage",
  },
  {
    path: "form-grid-gutters",
    component: "FormGridGuttersPage",
  },
  {
    path: "form-input-groups",
    component: "FormInputGroupsPage",
  },
  {
    path: "form-mask",
    component: "FormMaskPage",
  },
  {
    path: "form-pickers",
    component: "FormPickersPage",
  },
  {
    path: "form-select",
    component: "FormSelectPage",
  },
  {
    path: "form-select2",
    component: "FormSelect2Page",
  },
  {
    path: "form-validation",
    component: "FormValidationPage",
  },
  {
    path: "form-wizard",
    component: "FormWizardPage",
  },
  {
    path: "gallery",
    component: "GalleryPage",
  },
  {
    path: "gdpr",
    component: "GdprPage",
  },
  {
    path: "goal-tracking",
    component: "GoalTrackingPage",
  },
  {
    path: "goal-type",
    component: "GoalTypePage",
  },
  {
    path: "holidays",
    component: "HolidaysPage",
  },
  {
    path: "icon-bootstrap",
    component: "IconBootstrapPage",
  },
  {
    path: "icon-feather",
    component: "IconFeatherPage",
  },
  {
    path: "icon-flag",
    component: "IconFlagPage",
  },
  {
    path: "icon-fontawesome",
    component: "IconFontawesomePage",
  },
  {
    path: "icon-ionic",
    component: "IconIonicPage",
  },
  {
    path: "icon-material",
    component: "IconMaterialPage",
  },
  {
    path: "icon-pe7",
    component: "IconPe7Page",
  },
  {
    path: "icon-remix",
    component: "IconRemixPage",
  },
  {
    path: "icon-simpleline",
    component: "IconSimplelinePage",
  },
  {
    path: "icon-tabler",
    component: "IconTablerPage",
  },
  {
    path: "icon-themify",
    component: "IconThemifyPage",
  },
  {
    path: "icon-typicon",
    component: "IconTypiconPage",
  },
  {
    path: "icon-weather",
    component: "IconWeatherPage",
  },
  {
    path: "incoming-call",
    component: "IncomingCallPage",
  },
  {
    path: "invoice-report",
    component: "InvoiceReportPage",
  },
  {
    path: "estimates",
    component: "EstimatesPage",
  },
  {
    path: "expenses",
    component: "ExpensesPage",
  },
  {
    path: "expenses-report",
    component: "ExpensesReportPage",
  },
  {
    path: "employee-salary",
    component: "EmployeeSalaryPage",
  },
  {
    path: "payslip",
    component: "PayslipPage",
  },
  {
    path: "payroll",
    component: "PayrollPage",
  },
  {
    path: "provident-fund",
    component: "ProvidentFundPage",
  },
  {
    path: "taxes",
    component: "TaxesPage",
  },
  {
    path: "payments",
    component: "PaymentsPage",
  },
  {
    path: "categories",
    component: "CategoriesPage",
  },
  {
    path: "budgets",
    component: "BudgetsPage",
  },
  {
    path: "budget-expenses",
    component: "BudgetExpensesPage",
  },
  {
    path: "budget-revenues",
    component: "BudgetRevenuesPage",
  },
  {
    path: "invoices",
    component: "InvoicesPage",
  },
  {
    path: "invoice",
    component: "InvoicePage",
  },
  {
    path: "invoice-details",
    component: "InvoiceDetailsPage",
  },
  {
    path: "job-grid",
    component: "JobGridPage",
  },
  {
    path: "job-list",
    component: "JobListPage",
  },
  {
    path: "job-details",
    component: "JobDetailsPage",
  },
  {
    path: "job-category",
    component: "JobCategoryPage",
  },
  {
    path: "invoice-settings",
    component: "InvoiceSettingsPage",
  },
  {
    path: "kanban-view",
    component: "KanbanViewPage",
  },
  {
    path: "knowledgebase",
    component: "KnowledgebasePage",
  },
  {
    path: "language",
    component: "LanguagePage",
  },
  {
    path: "leads",
    component: "LeadsPage",
  },
  {
    path: "leads/:id",
    component: "LeadEditPage",
  },
  {
    path: "leads/:id/chat",
    component: "LeadChatPage",
  },
  {
    path: "leads-dashboard",
    component: "LeadsDashboardPage",
  },
  {
    path: "lead-chats",
    component: "LeadChatsPage",
  },
  {
    path: "rejected-leads",
    component: "RejectedLeadsPage",
  },
  {
    path: "leave-report",
    component: "LeaveReportPage",
  },
  {
    path: "leave-settings",
    component: "LeaveSettingsPage",
  },
  {
    path: "leaves",
    component: "LeavesPage",
  },
  {
    path: "leaves-employee",
    component: "LeavesEmployeePage",
  },
  {
    path: "localization-settings",
    component: "LocalizationSettingsPage",
  },
  {
    path: "lock-screen",
    component: "LockScreenPage",
  },
  {
    path: "login",
    component: "LoginPage",
  },
  {
    path: "login-2",
    component: "Login2Page",
  },
  {
    path: "login-3",
    component: "Login3Page",
  },
  {
    path: "logs",
    component: "LogsPage",
  },
  {
    path: "flow",
    component: "FlowPage",
  },
  {
    path: "maintenance-mode",
    component: "MaintenanceModePage",
  },
  {
    path: "maps-leaflet",
    component: "MapsLeafletPage",
  },
  {
    path: "maps-vector",
    component: "MapsVectorPage",
  },
  {
    path: "notes",
    component: "NotesPage",
  },
  {
    path: "notification-settings",
    component: "NotificationSettingsPage",
  },
  {
    path: "otp-settings",
    component: "OtpSettingsPage",
  },
  {
    path: "outgoing-call",
    component: "OutgoingCallPage",
  },
  {
    path: "overtime",
    component: "OvertimePage",
  },
  {
    path: "packages",
    component: "PackagesPage",
  },
  {
    path: "pages",
    component: "PagesPage",
  },
  {
    path: "payment-gateways",
    component: "PaymentGatewaysPage",
  },
  {
    path: "payment-report",
    component: "PaymentReportPage",
  },
  {
    path: "payments",
    component: "PaymentsPage",
  },
  {
    path: "payroll",
    component: "PayrollPage",
  },
  {
    path: "payslip",
    component: "PayslipPage",
  },
  {
    path: "payslip-report",
    component: "PayslipReportPage",
  },
  {
    path: "performance-appraisal",
    component: "PerformanceAppraisalPage",
  },
  {
    path: "performance-indicator",
    component: "PerformanceIndicatorPage",
  },
  {
    path: "pipeline",
    component: "PipelinePage",
  },
  {
    path: "policy",
    component: "PolicyPage",
  },
  {
    path: "preferences",
    component: "PreferencesPage",
  },
  {
    path: "prefixes",
    component: "PrefixesPage",
  },
  {
    path: "pricing",
    component: "PricingPage",
  },
  {
    path: "privacy-policy",
    component: "PrivacyPolicyPage",
  },
  {
    path: "profile",
    component: "ProfilePage",
  },
  {
    path: "profile-settings",
    component: "ProfileSettingsPage",
  },
  {
    path: "project-report",
    component: "ProjectReportPage",
  },
  {
    path: "projects-grid",
    component: "ProjectsPage",
  },
  {
    path: "promotion",
    component: "PromotionPage",
  },
  {
    path: "provident-fund",
    component: "ProvidentFundPage",
  },
  {
    path: "purchase-transaction",
    component: "PurchaseTransactionPage",
  },
  {
    path: "referals",
    component: "ReferalsPage",
  },
  {
    path: "register",
    component: "RegisterPage",
  },
  {
    path: "register-2",
    component: "Register2Page",
  },
  {
    path: "register-3",
    component: "Register3Page",
  },
  {
    path: "registration",
    component: "RegistrationSettingsPage",
  },
  {
    path: "reset-password",
    component: "ResetPasswordPage",
  },
  {
    path: "reset-password-2",
    component: "ResetPassword2Page",
  },
  {
    path: "reset-password-3",
    component: "ResetPassword3Page",
  },
  {
    path: "promotion",
    component: "PromotionPage",
  },
  {
    path: "resignation",
    component: "ResignationPage",
  },
  {
    path: "referrals",
    component: "ReferralsPage",
  },
  {
    path: "refferals",
    component: "ReferralsPage",
  },
  {
    path: "roles-permissions",
    component: "RolesPermissionsPage",
  },
  {
    path: "salary-settings",
    component: "SalarySettingsPage",
  },
  {
    path: "schedule-timing",
    component: "ScheduleTimingPage",
  },
  {
    path: "search-result",
    component: "SearchResultPage",
  },
  {
    path: "security",
    component: "SecurityPage",
  },
  {
    path: "security-settings",
    component: "SecuritySettingsPage",
  },
  {
    path: "seo-settings",
    component: "SeoSettingsPage",
  },
  {
    path: "session-settings",
    component: "SessionSettingsPage",
  },
  {
    path: "sms-settings",
    component: "SmsSettingsPage",
  },
  {
    path: "sms-template",
    component: "SmsTemplatePage",
  },
  {
    path: "social-feed",
    component: "SocialFeedPage",
  },
  {
    path: "starter",
    component: "StarterPage",
  },
  {
    path: "states",
    component: "StatesPage",
  },
  {
    path: "storage-settings",
    component: "StorageSettingsPage",
  },
  {
    path: "subscription",
    component: "SubscriptionPage",
  },
  {
    path: "tables-basic",
    component: "TablesBasicPage",
  },
  {
    path: "task-board",
    component: "TaskBoardPage",
  },
  {
    path: "task-report",
    component: "TaskReportPage",
  },
  {
    path: "tasks",
    component: "TasksPage",
  },
  {
    path: "taxes",
    component: "TaxesPage",
  },
  {
    path: "tax-rates",
    component: "TaxRatesPage",
  },
  {
    path: "termination",
    component: "TerminationPage",
  },
  {
    path: "terms-condition",
    component: "TermsConditionPage",
  },
  {
    path: "testimonials",
    component: "TestimonialsPage",
  },
  {
    path: "tickets",
    component: "TicketsPage",
  },
  {
    path: "timeline",
    component: "TimelinePage",
  },
  {
    path: "timesheets",
    component: "TimesheetsPage",
  },
  {
    path: "todo",
    component: "TodoPage",
  },
  {
    path: "trainers",
    component: "TrainersPage",
  },
  {
    path: "training",
    component: "TrainingPage",
  },
  {
    path: "training-type",
    component: "TrainingTypePage",
  },
  {
    path: "two-step-verification",
    component: "TwoStepVerificationPage",
  },
  {
    path: "two-step-verification-2",
    component: "TwoStepVerification2Page",
  },
  {
    path: "two-step-verification-3",
    component: "TwoStepVerification3Page",
  },
  {
    path: "ui-accordion",
    component: "UiAccordionPage",
  },
  {
    path: "ui-alerts",
    component: "UiAlertsPage",
  },
  {
    path: "ui-avatar",
    component: "UiAvatarPage",
  },
  {
    path: "ui-badges",
    component: "UiBadgesPage",
  },
  {
    path: "ui-borders",
    component: "UiBordersPage",
  },
  {
    path: "ui-breadcrumb",
    component: "UiBreadcrumbPage",
  },
  {
    path: "ui-buttons",
    component: "UiButtonsPage",
  },
  {
    path: "ui-buttons-group",
    component: "UiButtonsGroupPage",
  },
  {
    path: "ui-cards",
    component: "UiCardsPage",
  },
  {
    path: "ui-carousel",
    component: "UiCarouselPage",
  },
  {
    path: "ui-clipboard",
    component: "UiClipboardPage",
  },
  {
    path: "ui-colors",
    component: "UiColorsPage",
  },
  {
    path: "ui-counter",
    component: "UiCounterPage",
  },
  {
    path: "ui-drag-drop",
    component: "UiDragDropPage",
  },
  {
    path: "ui-dropdowns",
    component: "UiDropdownsPage",
  },
  {
    path: "ui-grid",
    component: "UiGridPage",
  },
  {
    path: "ui-images",
    component: "UiImagesPage",
  },
  {
    path: "ui-lightbox",
    component: "UiLightboxPage",
  },
  {
    path: "ui-media",
    component: "UiMediaPage",
  },
  {
    path: "ui-modals",
    component: "UiModalsPage",
  },
  {
    path: "ui-nav-tabs",
    component: "UiNavTabsPage",
  },
  {
    path: "ui-offcanvas",
    component: "UiOffcanvasPage",
  },
  {
    path: "ui-pagination",
    component: "UiPaginationPage",
  },
  {
    path: "ui-placeholders",
    component: "UiPlaceholdersPage",
  },
  {
    path: "ui-popovers",
    component: "UiPopoversPage",
  },
  {
    path: "ui-progress",
    component: "UiProgressPage",
  },
  {
    path: "ui-rangeslider",
    component: "UiRangesliderPage",
  },
  {
    path: "ui-rating",
    component: "UiRatingPage",
  },
  {
    path: "ui-ribbon",
    component: "UiRibbonPage",
  },
  {
    path: "ui-scrollbar",
    component: "UiScrollbarPage",
  },
  {
    path: "ui-sortable",
    component: "UiSortablePage",
  },
  {
    path: "ui-spinner",
    component: "UiSpinnerPage",
  },
  {
    path: "ui-stickynote",
    component: "UiStickynotePage",
  },
  {
    path: "ui-sweetalerts",
    component: "UiSweetalertsPage",
  },
  {
    path: "ui-swiperjs",
    component: "UiSwiperjsPage",
  },
  {
    path: "ui-text-editor",
    component: "UiTextEditorPage",
  },
  {
    path: "ui-timeline",
    component: "UiTimelinePage",
  },
  {
    path: "ui-toasts",
    component: "UiToastsPage",
  },
  {
    path: "ui-tooltips",
    component: "UiTooltipsPage",
  },
  {
    path: "ui-typography",
    component: "UiTypographyPage",
  },
  {
    path: "ui-video",
    component: "UiVideoPage",
  },
  {
    path: "under-construction",
    component: "UnderConstructionPage",
  },
  {
    path: "under-maintenance",
    component: "UnderMaintenancePage",
  },
  {
    path: "useradmin",
    component: "UseradminPage",
  },
  {
    path: "user-edit/:id",
    component: "UserEditPage",
  },
  {
    path: "usergroups",
    component: "UsergroupsPage",
  },
  {
    path: "user-report",
    component: "UserReportPage",
  },
  {
    path: "users",
    component: "UsersPage",
  },
  {
    path: "user-settings",
    component: "UserSettingsPage",
  },
  {
    path: "video-call",
    component: "VideoCallPage",
  },
  {
    path: "voice-call",
    component: "VoiceCallPage",
  },
  {
    path: "rrq",
    component: "RrqPage",
  },
  {
    path: "rrq-type",
    component: "RrqTypePage",
  },
  {
    path: "projects-page",
    component: "ProjectsPage",
  },
  {
    path: "project-status",
    component: "ProjectStatusPage",
  },
  {
    path: "project-type",
    component: "ProjectTypePage",
  },
  {
    path: "channel-partners",
    component: "ChannelPartnersPage",
  },
  {
    path: "channel-partners/create",
    component: "CreateChannelPartnerPage",
  },
  {
    path: "channel-partners/:id",
    component: "ChannelPartnerEditPage",
  },
  {
    path: "primary-source",
    component: "PrimarySourcePage",
  },
  {
    path: "lead-status",
    component: "LeadStatusPage",
  },
  {
    path: "secondary-source",
    component: "SecondarySourcePage",
  },
  {
    path: "tertiary-source",
    component: "TertiarySourcePage",
  },
  {
    path: "lead-type",
    component: "LeadTypePage",
  },
  {
    path: "usergroups/edit/:id",
    component: "UserGroupEditPage",
  },
];
